...
export const routeTransitionAnimations = trigger('triggerName', [transition('One => Two, Two => Three, One => Three', [   
        ...
        group([
         ...
         query(':enter', animateChild())
       ]),transition('Three => ThreeDetails, Two => TwoDetails', [   
        style({ position: 'relative' }),
        query(':enter, :leave', [
          style({
            position: 'absolute',
        ...
        query(':leave', animateChild()),
        group([
          query(':leave', [animate('0.3s ease-out', style({ left: '0%', opacity: 0 }))]),
          query(':enter', [animate('0.3s ease-out', style({ left: '0%', opacity: 1 }))])
         ]),
         query(':enter', animateChild())
       ]),transition('ThreeDetails => Three, ThreeDetails => Two, ThreeDetails => one,TwoDetails => Three, TwoDetails => Two, TwoDetails => one', [   
        style({ position: 'relative' }),
        query(':enter, :leave', [
          style({
            position: 'absolute',
